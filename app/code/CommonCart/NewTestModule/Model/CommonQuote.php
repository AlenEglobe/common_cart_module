...
namespace CommonCart\NewTestModule\Model;
...
class CommonQuote extends CheckoutSession
{
        ...
        if ($this->_quote === null) {
            ...
            if ($this->getQuoteId()) {
                try {
                    ...
                    if ($this->checkActiveAdminUser()) {
                        $adminId = $this->checkActiveAdminUser();
    ...
    /**
     * Check if the common cart attribute is set for the customer.
     *
     * @param int $customerId
     * @return int|null
     */
    public function checkCommonCartAttribute(int $customerId): ?int
    {
        try {
            // Load customer by ID
            $customer = $this->customerRepository->getById($customerId);

            // Check if the custom attribute value is set for the customer
            $customAttributeValue = $customer->getCustomAttribute('common_cart')->getValue(); // Replace 'CustomAttributeName' with your attribute code
            if ($customAttributeValue) {
                // Custom attribute value is set for the customer
                return $customerId;
            } else {
                // Custom attribute value is not set for the customer
                return null;
            }
        } catch (\Exception $e) {
            // Handle exceptions if any
            // Log or throw exception, depending on your requirement
            return null;
        }
    }

    /**
     * Check if the logged-in user is an active admin user.
     *
     * @return bool
     */
    public function checkActiveAdminUser(): bool
    {
        $customerId = $this->_customerSession->getCustomerId();
        $company = $this->companyManagement->getByCustomerId($customerId);
        if ($company != null) {
            $adminId = $company->getSuperUserId();
            return $this->checkCommonCartAttribute($adminId);
        }
        return false;
    }
}
